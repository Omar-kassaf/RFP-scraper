...
def check_user_task(username):
    return user_data[username]["progress"].get("task_complete", True)
...
def is_task_in_progress(username):
    return not user_data[username]["progress"]["task_complete"]

def extract_keywords_from_file(uploaded_file):
    # (Unchanged code for extracting keywords from uploaded files)
    pass
...
def show_email_input_page():
    ...
    if st.button("Submit"):
        ...
        save_user_progress(st.session_state["username"], all_keywords, st.session_state["selected_option"], False)
...
def process_request(emails, keywords, selected_option, username):
    try:
        ...
        if not os.path.isfile(file_name):
            ...
            save_user_progress(username, keywords, selected_option, True)
        ...
        save_user_progress(username, keywords, selected_option, True)  # Mark task as failed
